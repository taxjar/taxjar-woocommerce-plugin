...
if ( ! class_exists( 'WC_Taxjar_Integration' ) ) :
	...
	class WC_Taxjar_Integration extends WC_Settings_API {
		 ...
		 * Add Settings Page.
		 *
		 * @param [type] $settings_tabs Settings tabs.
		 */
		...
		public function calculate_tax( $options = array() ) {
			...
			if ( is_object( WC()->customer ) && WC()->customer->is_vat_exempt() ) {
				...
				foreach ( $taxes['line_items'] as $line_item_key => $line_item ) {
					...
					if ( $line_item->combined_tax_rate && 0 !== $line_item->combined_tax_rate ) {
		 ...
		 *
		 * @param  [type] $id         ID.
		 * @param  [type] $line_items Line Items.
		 */
		 ...
		 *
		 * @param  [type] $taxes Taxes.
		 * @param  [type] $price Price.
		 * @param  [type] $rates Rates.
		 * @return array
		 */
		 ...
		 *
		 * @param  [type] $to_country To Country.
		 * @param  [type] $to_state   To State.
		 * @param  [type] $to_zip     To Zip.
		 */
		...
		/**
		 * Is valid Exemption Type.
		 *
		 * @param  [type] $exemption_type Exemption Type.
		 * @return boolean
		 */
